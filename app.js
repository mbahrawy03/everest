...
function colorNameToHex(name) {
  const map = {
    'black':'#1a1a1a','white':'#f5f5f5','grey':'#888','gray':'#888',
    'beige':'#c9b99a','sand':'#d4c4a0','khaki':'#8b7355','olive':'#6b6b3a',
    'navy':'#1a2744','blue':'#2563eb','red':'#dc2626','green':'#16a34a',
    'pink':'#ec4899','purple':'#9333ea','orange':'#ea580c','yellow':'#ca8a04',
    'brown':'#78350f','cream':'#fef9ef','camel':'#c19a6b','rust':'#b45309',
  };
...
function openProduct(id) {
  ...
  // Images gallery
  const images = currentProduct.images?.length > 0 ? currentProduct.images : (currentProduct.image_url ? [currentProduct.image_url] : []);
...
async function handleCheckout() {
  ...
  if (!currentUser) { closeCart(); openAuth(); alert('Please login to complete your order.'); return; }
  ...
  const { error } = await db.from('orders').insert({ customer_id: currentUser.id, items: orderItems, total, status: 'pending' });
  if (error) { alert('Could not place order. Please try again.'); console.error(error); return; }
